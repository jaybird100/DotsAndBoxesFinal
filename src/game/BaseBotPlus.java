...
public class BaseBotPlus {
    public void getEdge(){
        BaseBotNode state;
        if(player1Turn){
            state = new BaseBotNode(Node.matrixCopy(Graph.getMatrix()), Graph.getPlayer1Score(), Graph.getPlayer2Score(), Node.avCopy(Graph.getAvailableLines()), true, false, false, null, doubleCross);
        }else{
            state = new BaseBotNode(Node.matrixCopy(Graph.getMatrix()), Graph.getPlayer2Score(), Graph.getPlayer1Score(), Node.avCopy(Graph.getAvailableLines()), true, false, false, null,doubleCross);
        }
        double max=-1*Double.MAX_VALUE;
        ArrayList<ELine> options = new ArrayList<>();
        for (ELine v : Graph.availableLines) {
            BaseBotNode temp = state.performMove(v);
